...
- Palette: `static/css/connected-theme.css` and `connected-open-source.css`, using the Railgrid app dark/light tokens. Shared composition lives in `connected-base.css` and `connected-layers.css`.
...
`site-theme.js` applies the preference before paint on marketing and docs pages. The canonical key is `railgrid-theme` (light, dark, or system). Migration prefers an existing canonical choice, then `railgrid-connected-theme`, then `railgrid-docs-theme`, with dark as the new-visitor default. The shared picker uses `connected-theme.js`; docs no longer has an independent theme controller. System changes and storage events synchronize the resolved palette. Legacy `.dark` is a projection of the shared choice for existing docs syntax tokens, not a separate preference.

Docs use the marketing identity in a single header with documentation section links, search, the shared theme picker, and Open console. At narrow widths, a documentation-only menu replaces the section links. The Railgrid logo links home; the Docs link and compact menu provide access to the documentation landing page. Section popovers, search dialog, sidebar, article tools, and code controls remain available. `connected-docs.css` supplies the reading-specific typography and token aliases; a ResizeObserver measures the header for sticky navigation and anchor offsets.
