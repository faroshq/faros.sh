...
The [aggregate MCP endpoint](/docs/use/mcp/) uses the `databricks__` prefix. These tools operate on tables already imported into the active Faros workspace, subject to its permissions and the upstream Databricks connection.
...
[Resource fields and validation rules](/docs/use/databricks/schemas/) are generated from the checked-in schemas, with a downloadable JSON bundle.
