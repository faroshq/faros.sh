...
`kubectl railgrid ssh` opens a shell (or runs a single command) on a `server`-type edge. The connection rides on the same reverse tunnel the agent uses to talk to the hub — no inbound SSH port, no public IP, no jump host.

## Interactive shell

```bash
kubectl railgrid ssh my-vps
```

The CLI looks up the edge's endpoint on the hub, opens a WebSocket with your bearer token (from `kubectl railgrid login`), puts your terminal in raw mode, and streams the session — including window-resize events — through the agent's reverse tunnel.
...
```bash
kubectl railgrid ssh my-vps -- df -h
kubectl railgrid ssh my-vps -- "systemctl status nginx"
```
...
There's no `railgrid scp`, but you can pipe through the SSH stream:

```bash
# Local to remote
cat localfile.txt | kubectl railgrid ssh my-vps -- "cat > /tmp/remotefile.txt"

# Remote to local
kubectl railgrid ssh my-vps -- cat /etc/hostname > local-hostname.txt
```
...
- Port forwarding (`-L` / `-R`) is not exposed through `kubectl railgrid ssh`. If you need it, run a real SSH server on the host and reach it some other way.
