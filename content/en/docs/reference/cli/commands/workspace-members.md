---
title: "railgrid workspace members"
linkTitle: "workspace members"
description: "List and change who has access to the workspace"
weight: 73
doc_type: Reference
generated: true
---
<!-- Generated by scripts/sync-cli-docs.py from railgrid/railgrid docs/cli. Do not edit by hand; run `make sync-cli-docs`. -->

## Synopsis

Membership is the RBAC unit of a railgrid workspace: every member is either an
admin (may manage members and settings) or a member. Admins of an
organization can manage every workspace in it.

```bash
railgrid workspace members                     # who has access, and as what
railgrid workspace members add alice@example.com --role member
railgrid workspace members add bob@example.com --role admin --invite   # not signed up yet
railgrid workspace members set-role alice@example.com admin
railgrid workspace members remove bob@example.com
```

```bash
railgrid workspace members [flags]
```

## Options

```text
  -h, --help            help for list
  -o, --output string   Output format: json, yaml, name
```

## Inherited options

```text
      --insecure-skip-tls-verify   Skip TLS certificate verification when talking to the hub
      --kubeconfig string          Path to the kubeconfig file (default: $KUBECONFIG, then ~/.kube/config)
      --org string                 Organization display name or UUID (default: the org that owns the kubeconfig's workspace)
      --workspace string           Workspace display name or UUID (default: the workspace the kubeconfig points at)
```

## See also

- [railgrid workspace](/docs/reference/cli/commands/workspace/) — Workspaces of an organization, and who is in them
- [railgrid workspace members add](/docs/reference/cli/commands/workspace-members-add/) — Add a member to the workspace (admin only)
- [railgrid workspace members list](/docs/reference/cli/commands/workspace-members-list/) — List workspace members
- [railgrid workspace members remove](/docs/reference/cli/commands/workspace-members-remove/) — Remove a member from the workspace (admin only)
- [railgrid workspace members set-role](/docs/reference/cli/commands/workspace-members-set-role/) — Change a member's role in the workspace (admin only)
