...
[Download complete schemas](/schemas/kuery.json), including nested validation rules and status definitions. This page covers Kubernetes-style resources; provider HTTP actions and runtime behavior are separate contracts. Return to [API reference](/docs/reference/providers/kuery/) for those interfaces and related guides.
...
| `status.lastOpenedAt` | string | No | No description supplied by the source schema. |
