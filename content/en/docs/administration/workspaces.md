...
- A **workspace** is the unit of isolation: your edges, enabled providers, and resources live in exactly one workspace. Under the hood each workspace is its own kcp logical cluster in the hub's workspace tree (`root:railgrid:tenants:<org>:<workspace>`), so isolation is structural, not filter-based.
- Workspaces can contain **edge mounts** — the per-edge sub-workspaces `railgrid connect` navigates into.

![Choose an organization, create a workspace, review its members, and switch into it.](/images/docs/console/tenancy.gif)

The CLI drives all of this with [`railgrid use`](/docs/reference/cli/workspaces/) (pick org + workspace) and the portal has an equivalent switcher.
...
- Members can be added using existing users. Some console flows support pending users invited by email; verify the supported identity-provider flow on your deployment. See [Add a teammate](/docs/administration/members/).
...
- A Kubernetes ServiceAccount living in the workspace, labeled as railgrid-managed.
- A token (audience `railgrid`), returned **once** at creation — store it then.
...
2. The active org/workspace ride along as `X-Railgrid-Org` / `X-Railgrid-Workspace` headers — the CLI and portal set them from your `railgrid use` selection.
