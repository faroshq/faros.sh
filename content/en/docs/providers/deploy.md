...
`values.yaml` highlights: `image.*`, `replicaCount` (2 by default — drop to 1 only if you terminate [revdial tunnels](/docs/providers/connectivity/#the-revdial-reverse-tunnel)), `service.port`, `hub.url`, `hub.tokenSecretRef`, `providerKubeconfig.secretName`, `catalogEntry.enabled`.
