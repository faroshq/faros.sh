...
The chart README contains the matching values table and image coordinates for the deployed source version: [chart instructions](https://github.com/railgrid/railgrid/blob/main/providers/edges/deploy/chart/README.md). Replace the placeholder kubeconfig and hub URL; do not put bearer tokens or database URLs directly in a production values file.
...
Run `kubectl -n railgrid-provider-edges get pods` and wait for Ready and a fresh heartbeat. Enable Edges in a test workspace, connect one disposable agent, and verify reconnect plus kubectl/SSH after a pod restart using the [Edges quickstart](/docs/use/edges/quickstart/).
...
[Chart values](https://github.com/railgrid/railgrid/blob/main/providers/edges/deploy/chart/values.yaml) are authoritative for this source baseline. Keep the deployed image and chart versions recorded together.
