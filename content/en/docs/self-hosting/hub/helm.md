...
An image error calls for checking the image tag and kind load; a pending PVC calls for checking storage provisioning. Use [operations and recovery](/docs/self-hosting/hub/operations/) for further diagnosis.
...
For an existing hosting cluster, prepare DNS, [ingress and TLS](/docs/self-hosting/hub/ingress/), and [OIDC](/docs/self-hosting/hub/oidc/) first. Use a reviewed chart checkout and a corresponding hub image available to your cluster. The example below is a configuration starting point; substitute your own hostname, identity provider, administrator, TLS Secret, and image tag.
