...
3. **Honour forwarded identity.** Every MCP request arrives through the backend proxy with `X-Railgrid-Tenant`, `X-Railgrid-Cluster`, and the caller's `Authorization` bearer. Do all tenant work *as that token, scoped to that workspace* — never your provider SA. This is [pattern B](/docs/extend/virtual-workspace/#the-two-identity-patterns); an MCP tool must never see more than its caller could with kubectl.
