...
Tenants need permission to bind your export. `init` creates it: a ClusterRole/ClusterRoleBinding pair `railgrid:providers:bind:<export-name>` in your workspace granting the `bind` verb on your APIExport to `system:authenticated`. Any authenticated railgrid user may bind — the platform admin gates *which providers exist* at onboarding time, and each tenant gates *whether to enable* per workspace.
...
- `tenantScoped: true` claims are bounded to the tenant's own workspace and auto-acceptable. Anything else requires the platform-admin override annotation (`railgrid.ai/accept-untrusted-claims`) — treat that as a red flag in review.
- Claims on `*.railgrid.ai` groups need the claimed export's `identityHash` (admin-supplied at deploy time); built-in Kubernetes types don't.
...
- **`edgeProxyAccess`** — the hub grants your provider SA (as a cluster-qualified subject) the `proxy` verb on `edges.railgrid.ai` resources in the tenant workspace. Disable always removes the grant.

## Anti-spoofing

The backend proxy **strips inbound `X-Railgrid-*` headers** before injecting verified values. Corollaries for your code:

- Trust `X-Railgrid-User` / `X-Railgrid-Tenant` / `X-Railgrid-Cluster` only on traffic that arrived through the hub proxy.
