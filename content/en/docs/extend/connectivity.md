...
Your handler's contract: trust the `X-Faros-*` headers (they're hub-asserted), act as the caller using the bearer token ([pattern B](/docs/extend/virtual-workspace/#the-two-identity-patterns)), and never accept those headers from any other path into your process.
