...
- The proxy injects `X-Railgrid-Base-Path` so your server can render relative asset paths if it needs to.
...
2. **Strips any inbound `X-Railgrid-*` headers** — a client can never forge identity.
3. Injects verified `X-Railgrid-User`, `X-Railgrid-Tenant` (workspace path), and `X-Railgrid-Cluster` (logical cluster ID).
...
Your handler's contract: trust the `X-Railgrid-*` headers (they're hub-asserted), act as the caller using the bearer token ([pattern B](/docs/extend/virtual-workspace/#the-two-identity-patterns)), and never accept those headers from any other path into your process.

A minimal backend endpoint (quickstart's `/api/hello`) just echoes `X-Railgrid-User` back — that's the whole integration test for the proxy contract.
...
> **Replica routing.** Edges can route requests to the replica holding an agent tunnel through a Lease registry and internal relay. Use the provider’s chart wiring and credentials; without replica routing, control connections and pickups must reach the same process. See [Edges self-hosting](/docs/self-hosting/providers/edges/).
...
```
/services/providers/edges/agent/{cluster}/apis/edges.railgrid.ai/v1alpha1/{resource}/{name}/proxy
