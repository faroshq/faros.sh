...
|:------|:-----------|:---------|:--------|
| `Provider` (`admin.railgrid.ai/v1alpha1`) | platform admin | `root:railgrid:system:providers` | Provisions the provider workspace, service account, and kubeconfig |
...
| `CatalogEntry` (`providers.railgrid.ai/v1alpha1`) | your `init` step | `root:railgrid:providers:<name>` | Routing, UI/backend URLs, Enable-dialog metadata |
...
1. Creates the workspace `root:railgrid:providers:<name>` with the restricted `provider` workspace type and waits for it to be ready.
...
4. Writes the kubeconfig into the Secret `<name>-kubeconfig` in `root:railgrid:system:providers`, and sets `status.workspacePath`, `status.workspaceCluster`, `status.secretRef`, and the `Ready` condition.
...
The `provider` workspace type is deliberately restricted: it can't create child workspaces, and its only default binding is `providers.railgrid.ai` — just enough for the provider to self-register its `CatalogEntry`.

## Step 2 — your `init` subcommand bootstraps the API

Your Deployment runs an initContainer with `args: ["init"]` and the minted kubeconfig mounted (`RAILGRID_PROVIDER_KUBECONFIG=/var/run/secrets/railgrid/railgrid-provider-kubeconfig`). The Railgrid provider SDK's `install.Bootstrap` then runs, idempotently and in order:

1. **Apply schemas** — every `*.yaml` in `RAILGRID_SCHEMAS_DIR` (default `/etc/railgrid/schemas`, baked into your image) is applied as an `APIResourceSchema`.
...
4. **Apply the bind grant** — a ClusterRole/Binding named `railgrid:providers:bind:<export>` letting `system:authenticated` create APIBindings to your export. Without it, every tenant Enable fails with a 403.
5. **Self-register the `CatalogEntry`** — from the file at `RAILGRID_CATALOGENTRY_FILE` (the Helm chart renders your `manifest.yaml` into a ConfigMap).
...
spec:
  ...
  ui:
    url: "http://quickstart.railgrid.svc.cluster.local:8081"
    indexPath: "/"
  backend:
    url: "http://quickstart.railgrid.svc.cluster.local:8081"
    healthPath: "/healthz"
...
| `ui.children[]` | Sub-navigation items (`{displayName, builtinRoute}`); your element reads the active child from `railgridContext.subPath`. |
...
From that moment your resources are ordinary objects in the tenant workspace — `kubectl get greetings` just works — and your controllers see them through the [virtual workspace](/docs/extend/virtual-workspace/).
